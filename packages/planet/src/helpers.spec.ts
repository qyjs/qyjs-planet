import {
    buildResourceFilePath, coerceArray, createElementByTemplate, getHTMLElement,
    getResourceFileName, getScriptsAndStylesFullPaths,
    getTagNameByTemplate, hashCode,
    isEmpty
} from './helpers';
...
describe('helpers', () => {
    ...
    describe('getScriptsAndStylesFullPaths', () => {
        ...
        it('should get correct full path with resourcePathPrefix', () => {
            const result = getScriptsAndStylesFullPaths({ ...app, resourcePathPrefix: '/app1/' });
            expect(result).toEqual({
                scripts: ['/app1/vendor.js', '/app1/main.js'],
                styles: ['/app1/styles/main.css']
            });
        });
        ...
        it('should get correct full path with manifest', () => {
            ...
            const result = getScriptsAndStylesFullPaths({ ...app, resourcePathPrefix: '/app1/' }, manifest);
            expect(result).toEqual({
                scripts: [`/app1/${manifest['vendor.js']}`, `/app1/${manifest['main.js']}`],
                styles: [`/app1/styles/${manifest['main.css']}`]
            });
        });
    });
});
